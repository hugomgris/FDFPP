/// @file
/// @brief Accompanying test code for test_2641[agxset]
...
int main(void) {
  ...
  // 1. a regular text attribute should be created as regular text
  {
    ...
    const int rc = agxset_text(g, attr, "bar value");
    assert(rc == 0);
  ...
  // 2. an HTML-like text attribute should be created as HTML-like
  {
    ...
    const int rc = agxset_html(g, attr, "baz value");
    assert(rc == 0);
  ...
  // 3. creating an attribute through the ambiguous `agxset` API should default
  // to regular text
  { // create an attribute initially as text
    ...
    const int rc = agxset(g, attr, "qux value");
    assert(rc == 0);
    const char *const value = agget(g, "qux");
    assert(value != NULL);
    assert(!aghtmlstr(value) && "agxset created a new attribute as HTML-like");
  }
  {
    ...
    assert(attr != NULL);
    // assign it a value, ambiguously
    const int rc = agxset(g, attr, "quux value");
    assert(rc == 0);
  ...
  // 4. creating an attribute through `agxset` using something we `agstrdup`ed
  ...
  { // create an attribute initially as text
    ...
    const int rc = agxset(g, attr, corge);
    assert(rc == 0);
    const char *const value = agget(g, "corge");
    assert(value != NULL);
    assert(!aghtmlstr(value) && "agxset did not detect agstrdup-ed value");
  }
  {
    // create an attribute initially as HTML-like
    Agsym_t *const attr =
    ...
    const int rc = agxset(g, attr, grault);
    assert(rc == 0);
  ...
  // 5. creating an attribute through `agxset` using something we
  ...
  { // create an attribute initially as text
    ...
    const int rc = agxset(g, attr, garply);
    assert(rc == 0);
    const char *const value = agget(g, "garply");
    assert(value != NULL);
    assert(aghtmlstr(value) && "agxset did not detect agstrdup_html-ed value");
  }
  {
    // create an attribute initially as HTML-like
    ...
    const int rc = agxset(g, attr, waldo);
    assert(rc == 0);
  ...
  // 6. creating an attribute through `agxset` using something we `agstrdup`ed
  ...
  // 7. creating an attribute through `agxset` using something we `agstrdup`ed
  // should create a regular text attribute, even when it aliases a previously
  ...
  // 8. creating an attribute through `agxset` using something we
  ...
  // 9. creating an attribute through `agxset` using something we
  // `agstrdup_html`ed should create an HTML-like attribute, even when it
